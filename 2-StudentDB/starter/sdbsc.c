...
int add_student(int fd, int id, char *fname, char *lname, int gpa){
    ...
    student_t *temp_student = malloc(STUDENT_RECORD_SIZE);
    ...
    temp_student->id = id;
    strcpy(temp_student->fname,  fname);
    strcpy(temp_student->lname,  lname);
    ...
    ssize_t bytes_written = write(fd, temp_student, STUDENT_RECORD_SIZE);
...
void print_student(student_t *s){
    ...
    {
        float calculated_gpa_from_s = s->gpa / 100.0f;
        printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST NAME", "LAST_NAME", "GPA");
